...
// this doesn't seem to work: we need a set comparison
m.add rule : ( sl(A,X) & sl(B,X) ) >> ~sl(A,B),  weight : 1
...
insert = data.getInserter(sl, readPart)
...
Database trainDB = data.getDatabase(writePart, [gene, ppiConnected, goCC, goBP, goMF] as Set, readPart);
